...
contains
  ...
  subroutine init_rand
    ...
    allocate (photL(2,7,nrays/4))
    allocate (photR(2,7,nrays/4))
    allocate (photB(2,7,nrays/4))
    allocate (photT(2,7,nrays/4))
    allocate (photO(2,7,nrays/4))
    allocate (photI(2,7,nrays/4))
  ...
  subroutine photons(xl0,yl0,zl0,xd,yd,zd,f)
    ...
    do while( f.ge.fmin  &
       ...
       !
       dtau=u(neqdin+1,i,j,k)*a0*dl*dx*rsc
       if (dtau < 1E-5) then
          ph(i,j,k)=f*a0*dl/((dx*rsc)**2)!*dx*rsc*/dx**3/rsc**3
          f=(1.-dtau)*f
       else
          ph(i,j,k)=ph(i,j,k)+f*(1.-exp(-dtau) )/(u(neqdin+1,i,j,k)*(dx*rsc)**3)
          f=f*exp(-dtau)
       end if
